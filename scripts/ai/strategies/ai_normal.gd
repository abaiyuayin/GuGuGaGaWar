...
## 普通 AI 策略（#需求5 用户拍板：普通模式随机出兵）
## 随机选择可负担的兵种，不分析克制、不算性价比，行为难以被玩家预判；
## 战役模式下保留「领先一个 tier」的兵种投放（敌方编成与玩家解锁进度挂钩）。

## 节点就绪时自动调用，设置普通 AI 的评估间隔
func _init() -> void:  ## 重写构造函数
	## 普通 AI 评估间隔为 1 秒（介于简单 2s 与困难 0.8s 之间）
	evaluation_interval = 1.0  ## 设置评估间隔为 1 秒

## 执行 AI 评估的核心方法
## 随机选择一个当前金币可负担的兵种；战役模式下 40% 概率优先玩家尚未解锁的兵种
## _ai_controller: AI 控制器节点引用（普通 AI 中未使用）
func _do_evaluate(_ai_controller: Node) -> void:  ## 重写评估方法
	...
	## 从可负担的兵种中随机选择一个（#需求5：普通模式随机出兵，不再死磕性价比之王 G2）
